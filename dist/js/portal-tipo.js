/*!
 * Portal Tipo v0.1.0 (https://portal-tipo.gub.uy)
 * 2017 - AGESIC
 */
/**
...
/**
 * Agrega clase svg / no-svg si el navegador soporta SVG o no
 */
...
/**
 * Si no soporta SVG reemplaza la extension por PNG
 * IMPORTANTE: Siempre que se usa SVG debe existir un PNG identico
 */
...
if (!svgAsImg()) {
  ...
  for (var i = 0, n = e.length; i < n; i++) {
    ...
    if (src.match(/svgz?$/)) {
      var pngFallback = src.substr(0, src.lastIndexOf('.')) + '.png';
      // var pngFallback = img.getAttribute('data-fallback');
      img.setAttribute('src', pngFallback);
    }
  }
}
...
if (typeof jQuery === 'undefined') {
  throw new Error('JavaScript de Portal Tipo requiere jQuery. Incluirlo antes de portal-tipo.js.');
}

/**
 * Prevenir click en anclas vacías (jQuery)
 */
...
/**
 * Responsive Menu with Search Box and top nav inside
 *
 * Este script evita duplicar contenido para mostrar correctamente el menu
 * en dispositivos moviles sin perder contenidos
 *
 * TODO: Cuando la pantalla es mayor a md remover lo que se agrego
 */
...
/**
 * Remover Top Nav y el subtitulo de lista agregado dentro del menu
 */
...
/**
 * Verificar que Search y Top Nav estan dentro del menu
 * Utiliza una clase en el tag html
 */

function isMenuAppend() {
  if ($('html').hasClass('menu-append')) {
    return true;
  }
...
/**
 * Click event del trigger del menu en moviles
 */

$(function () {
  $('.js-toggleMainNav').click(function () {
...
$.fn.accessibleMenu = function () {
  ...
  $('.Nav-item', el).mouseover(function () {
    ...
    $(this).find('> a').toggleAttr('aria-expanded', 'true', 'false');
    $(this).find('.' + subnavClass).toggleAttr('aria-hidden', 'true', 'false').toggleAttr('aria-expanded', 'true', 'false');
  }).mouseout(function () {
  ...
  $('a', el).focus(function () {
    $(this).parents('.' + itemClass).addClass(hoverClass).find('> a').toggleAttr('aria-expanded', 'true', 'false');
